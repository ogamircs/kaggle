...
crs$dataset <- read.csv(paste(HomeFolder,"compiled_full_train4.csv",sep=""), na.strings=c(".", "NA", "", "?"), strip.white=TRUE, encoding="UTF-8")
...
crs$numeric <- c("Month", "Week", "Size","IsHoliday","Type",
...
print("Building Generalized Boosted Regression Models ...")
...
crs$gbm <- gbm(Weekly_Sales ~ .,
	...
	cv.folds=10,
	shrinkage=0.001,
	weights=crs$dataset[,c(crs$weights)])
...
crs$testset <- read.csv(paste(HomeFolder,"compiled_test4.csv",sep=""), na.strings=c(".", "NA", "", "?"), header=TRUE, sep=",", encoding="UTF-8", strip.white=TRUE)
...
crs$pr <- predict(crs$gbm, newdata=crs$testset[,c(crs$input)])
...
write.csv(cbind(sdata, crs$pr), file=paste(HomeFolder,"compiled_test4_SCORES.csv",sep=""), row.names=FALSE)
...
crs$testset <- read.csv(paste(HomeFolder,"compiled_full_score4.csv",sep=""), na.strings=c(".", "NA", "", "?"), header=TRUE, sep=",", encoding="UTF-8", strip.white=TRUE)
...
# Output the combined data.
write.csv(cbind(sdata, crs$pr), file=paste(HomeFolder,"compiled_full_score4_SCORES.csv",sep=""), row.names=FALSE)
